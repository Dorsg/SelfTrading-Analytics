...
async def main() -> None:
    ...
    while True:
        ...
        if end_ts and ts > end_ts:
            log.info("Reached end of simulation at %s", datetime.fromtimestamp(ts, tz=timezone.utc))
            break
        try:
            await _tick_users(ts)
        except Exception:
            log.exception("Tick failed at %s", datetime.fromtimestamp(ts, tz=timezone.utc))
        ...
        for strat in strategies:
            for tf_val, tf_name in timeframes:
                ...
                try:
                    ...
                    continue
    if created:
        log.info("Bootstrapped %d runners for analytics user", created)
